  ...
  return (
    <div className="border-b border-gray-100 last:border-b-0">
      ...
      >
        ...
        <span className="mt-0.5 shrink-0 text-xs text-gray-500">{dialog.time}</span>
        <span className="mt-0.5 shrink-0 text-xs text-gray-400">
          {getDurationOrMsgsForDialog(dialog)}
        </span>
        <div className="min-w-0 flex-1">
          <p className={`text-sm text-gray-700 ${isExpanded ? "" : "line-clamp-1"}`}>
            {dialog.summary}
          </p>
        </div>
        <svg
      ...
      {/* Lead card in collapsed state — shown if dialog has leadCreated and is NOT expanded */}
      {!isExpanded && dialog.leadCreated && (
...
export function PanelA({ communication, onClose }: PanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Panel */}
      <div
        className={`absolute right-0 top-0 flex h-full w-[640px] flex-col border-l border-gray-200 bg-white shadow-xl transition-transform duration-300 ease-in-out ${
