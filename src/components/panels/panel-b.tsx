...
export function PanelB({ communication, onClose }: PanelProps) {
  ...
  // Animate in when communication is set
  useEffect(() => {
    if (communication) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Panel */}
      <div
        className={`absolute right-0 top-0 flex h-full w-[640px] flex-col bg-white shadow-xl transition-transform duration-200 ease-out ${
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {activeDialog && (
            <div className="flex flex-col gap-4">
              ...
              {/* Summary — always shown for every dialog tab */}
              <div className="rounded-lg bg-blue-50 border border-blue-100 p-4">
                <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-blue-500">Summary</h4>
                <p className="text-sm leading-relaxed text-gray-800">{activeDialog.summary}</p>
              </div>

              {/* Lead created card — shown for EVERY dialog that has leadCreated */}
